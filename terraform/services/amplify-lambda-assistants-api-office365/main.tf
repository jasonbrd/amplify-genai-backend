locals {
  service_name = "${var.name_prefix}-office365"
  src_dir      = "${path.module}/../../../amplify-lambda-assistants-api-office365"
  build_dir    = "${path.root}/build/amplify-lambda-assistants-api-office365"

  policy_name = "${local.service_name}-${var.stage}-iam-policy"

  extended_timeout_fns = toset(["microsoftIntegrationsRouter"])

  functions = {
    microsoftListIntegrations      = { handler = "integrations/oauth.get_integrations", timeout = 30, memory_size = 1024, kind = "http", path = "microsoft/integrations", method = "GET", cors = false }
    microsoftIntegrationsRouter    = { handler = "service/core.route_request", timeout = 300, memory_size = 1024, kind = "http", path = "microsoft/integrations/{proxy+}", method = "POST", cors = false }
    microsoftAdminConfigOpsTrigger = { handler = "service/register_ops.integration_config_trigger", timeout = 300, memory_size = 1024, kind = "stream", path = "", method = "", cors = false }
  }
...
resource "aws_lambda_event_source_mapping" "admin_config_ops" {
  ...
  function_name     = module.fn["microsoftAdminConfigOpsTrigger"].function_arn
